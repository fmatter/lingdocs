...
def table_label(string, mode="plural"):
    res = None
    if string.endswith(".csv"):
        res = string.replace(".csv", "")
    if string.endswith("Table"):
        res = string.replace("Table", "s").lower()
    if res:
        if mode == "singular":
            res = res.rstrip("s")
        return res
    raise ValueError(f"Cannot parse component name {string} (mode: {mode})")
